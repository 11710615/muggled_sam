...
from muggled_sam.v3_sam.components.video_postprocess import associate_det_trk
...
# 仅在与 detect_every_n_frames 对齐的「定时检测帧」上暂停；其余帧固定 waitKey(1) ms
...
def overlay_color_masks(frame, obj_masks, obj_color_map, alpha=0.35):
    """在原图上叠加多对象颜色 mask，并保持原图可见（与 video_segmentation_multiobj 一致）"""
    overlay = frame.copy()
...
def _mask_to_nchw(mask_tensor: torch.Tensor) -> torch.Tensor:
    """[H,W] or [1,H,W] -> [1,1,H,W] for interpolate"""
    t = mask_tensor.float()
    if t.dim() == 2:
...
def _mask_hw1(mask_tensor: torch.Tensor) -> torch.Tensor:
    """统一为 [1,H,W]，便于 findContours 与列表一致"""
    t = mask_tensor
    if t.dim() == 2:
        return t.unsqueeze(0)
    return t
...
try:
    ...
    for idx_frame in range(initial_frame_index, total_frames):
        ...
        objs_to_remove_list = []
        masks_on_frame_list = []  # 保存当前帧所有追踪对象的 mask [1,H,W]
        viz_mask_pairs = []  # (object_id, mask [1,H,W]) 与 multiobj 可视化对齐
        for idx_obj, obj_memory in memory_per_obj_dict.items():
            ...
            track_m = _mask_hw1(mask_preds[0, best_mask_idx, :, :])
            masks_on_frame_list.append(track_m)
            viz_mask_pairs.append((idx_obj, track_m))

        # Run detections to pick up new objects
        no_tracked_objects = len(memory_per_obj_dict) == 0
        scheduled_detection = ((idx_frame - initial_frame_index) % detect_every_n_frames) == 0
        need_detection = scheduled_detection or no_tracked_objects
        if need_detection:
            ...
            if num_detections > 0:
                ...
                for mask_tensor in masks_on_frame_list:
                    mask_uint8 = (mask_tensor.squeeze().detach().float().cpu().numpy() > 0).astype(np.uint8)
                ...
                for idx_offset, det_idx in enumerate(new_det_idxs_list):
                    ...
                    det_m = _mask_hw1(raw_det_mask)
                    masks_on_frame_list.append(det_m)
                    viz_mask_pairs.append((new_idx, det_m))
                pass
        ...
        # 彩色 mask 叠加 + 实例标签（与 video_segmentation_multiobj 一致）
        obj_masks = {}
        for idx_obj, mask_tensor in viz_mask_pairs:
            if idx_obj not in obj_color_map:
                obj_color_map[idx_obj] = obj_color_palette[len(obj_color_map) % len(obj_color_palette)]
            mt = _mask_to_nchw(mask_tensor)
            ...
            obj_masks[idx_obj] = (scaled_mask > 0.0).cpu().numpy().squeeze()

        overlay_frame = overlay_color_masks(display_frame, obj_masks, obj_color_map, alpha=0.35)
        draw_instance_labels(overlay_frame, obj_masks, obj_color_map)

        if highgui_available:
            try:
                cv2.imshow("Video Segmentation Result - q to quit", overlay_frame)
                # 固定 1ms；仅定时检测帧（且开启检测框可视化）才用长延迟，避免「无目标时每帧都检测」时卡死
                pause_on_detection_frame = scheduled_detection and enable_detection_visualization
                display_delay_ms = visualization_delay_ms if pause_on_detection_frame else 1
